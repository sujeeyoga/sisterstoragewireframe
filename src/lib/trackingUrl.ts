const encodedTrackingNumber = (trackingNumber: string) => encodeURIComponent(trackingNumber.trim());

export const getTrackingUrl = (carrier: string | null | undefined, trackingNumber: string): string => {
  const normalizedCarrier = carrier?.toLowerCase().replace(/[^a-z0-9]/g, '') ?? '';
  const encoded = encodedTrackingNumber(trackingNumber);
